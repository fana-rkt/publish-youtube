...
def upload_video():
    ...
    status = request.args.get("status", "unlisted")
                ...
                if db_row.recordid_db :
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            logger.info("Webhook successfully called for download progress.")
                        else:
                            logger.warning(f"Webhook responded with status: {response.status_code} for download progress.")
                    except Exception as e:
                        logger.warning(f"Error calling webhook for download progress. : {e}")

                else :
                    logger.warning("record_id empty")
...
def final_upload():
    ...
    thread = threading.Thread(target=upload_video_in_background, args=(db_row, file_id, creds_dict, session_id))
    thread.start()
